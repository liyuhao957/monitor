...
{{
  ...
  "content": "🚨 检测到加载器更新！\\n\\n| 类型 | 内容 |\\n|:------:|:------|\\n| 版本号 | `{{{{ new_version }}}}` |\\n| 规范版本 | `{{{{ new_spec }}}}` |\\n| 发现时间 | `{{{{ change_time }}}}` |\\n{{% if screenshot_url %}}\\n📸 [查看截图]({{{{ screenshot_url }}}})\\n{{% endif %}}",
  "summary": "华为快应用加载器版本更新通知模板",
  "required_fields": {{
    "old_version": "变化前的版本号",
    "new_version": "变化后的版本号",
    "old_spec": "变化前的规范版本",
    "new_spec": "变化后的规范版本",
    "change_time": "检测到变化的时间"
  }},
  "extraction_rules": {{
    "old_version": "从old_summary中提取V开头的版本号：V(\\d+\\.\\d+\\.\\d+\\.\\d+)",
    "new_version": "从new_summary中提取V开头的版本号：V(\\d+\\.\\d+\\.\\d+\\.\\d+)",
    "old_spec": "从old_summary中提取规范数字：支持(\\d+)规范",
    "new_spec": "从new_summary中提取规范数字：支持(\\d+)规范",
    "change_time": "使用当前时间：datetime.now().strftime('%Y-%m-%d %H:%M')"
  }}
}}"""
